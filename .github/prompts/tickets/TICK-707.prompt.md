...
# Ticket TICK-707 — HTTP gateway + D-Bus/XPC/COM (NATIVE N2/N3)

> **Wave 7** | **Domain:** Engine / Transport | **Depends on:** None
> **Source:** `docs/proposals/NATIVE_OS_API_REVIEW.md` §N2/N3, `docs/proposals/INSTALL_UPGRADE_LIFECYCLE.md` §N2
...
TICKET_ID: TICK-707
WAVE: 7
TITLE: HTTP gateway + D-Bus/XPC/COM (NATIVE N2/N3)
```

**Exclusive write files (SOLE writer for Wave 7):**
- `dataforge/api/transport/http_gateway.py [NEW FILE]`

**Read-only references (do not edit):**
- `docs/proposals/NATIVE_OS_API_REVIEW.md`
- `dataforge/api/transport/base.py`
- `dataforge/api/transport/uds.py`
- `dataforge/engine/daemon.py`

**Test target:** `tests/test_http_gateway.py [NEW FILE]`
**Validation:** `python -m pytest tests/test_http_gateway.py -q`
...
- `docs/proposals/NATIVE_OS_API_REVIEW.md` §N2/N3
- `docs/proposals/PERFORMANCE_INVESTIGATION.md` §D
...
architectural_context:
  ...
  breaking_changes: "None — additive transport, no existing API change"
requirements:
  summary: |
    Implement NATIVE N2/N3: HTTP gateway + D-Bus/XPC/COM transport. The current transport has UDS/Named Pipe (TICK-205) and daemon (TICK-301) but no HTTP gateway for remote or D-Bus/XPC/COM for native service activation.

    Create dataforge/api/transport/http_gateway.py with HttpGateway(Transport) that exposes FastAPI app with POST /jobs/scan, /jobs/search, /jobs/dupes etc. that proxy to EngineDaemon via JobQueue. For D-Bus/XPC/COM, create thin shims that register via dbus.service or pywin32 COM and forward to UDS. Ensure all transports share the same Job JSON schema from api/schema.py and that auto_discover tries HTTP as last fallback (already does).

    Keep dependencies optional: FastAPI/uvicorn only required if http_gateway used, dbus only on Linux, pywin32 only on Windows.
  source_documents:
    - "docs/proposals/NATIVE_OS_API_REVIEW.md"
    - "docs/proposals/INSTALL_UPGRADE_LIFECYCLE.md"
  acceptance_criteria:
    - "GIVEN HttpGateway started on 127.0.0.1:8765 WHEN POST /jobs/scan with JSON body THEN returns job_id and job can be polled via GET /jobs/{id}"
    - "GIVEN no FastAPI installed WHEN import http_gateway THEN raises informative ImportError with pip install hint, not crash"
    - "GIVEN D-Bus service file exists WHEN http_gateway tries D-Bus THEN falls back gracefully on non-Linux"
    - "GIVEN existing transport tests WHEN this change applied THEN still pass (no regression)"
verification:
  test_target: "tests/test_http_gateway.py [NEW FILE]"
  validation_command: "python -m pytest tests/test_http_gateway.py -q"
```
...
class HttpGateway(Transport):
    def __init__(self, daemon: EngineDaemon, host="127.0.0.1", port=8765): ...
    def run(self): ...  # uvicorn.run
```
