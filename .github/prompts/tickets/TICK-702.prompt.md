# Ticket TICK-702 — R-CORE-7: logger makedirs bare filename guard

> **Wave 7** | **Domain:** Core / Logger | **Depends on:** None
> **Source:** `docs/reviews/AUDIT_REPORT.md` Part 4 (R-CORE-7)
...
TICKET_ID: TICK-702
WAVE: 7
TITLE: R-CORE-7: logger makedirs bare filename guard
```
...
**Test target:** `tests/test_logger_bare_path.py [NEW FILE]`
**Validation:** `python -m pytest tests/test_logger_bare_path.py -q`
...
architectural_context:
  existing_symbols_to_use:
    - "logger.py: setup_logger, _dirname, os.makedirs"
  breaking_changes: "None — guard is additive, no API change"
requirements:
  summary: |
    Fix R-CORE-7: logger.py:142 `os.makedirs(os.path.dirname(log_file), exist_ok=True)` crashes with FileNotFoundError when log_file is a bare filename like "app.log" (dirname is ""), and also when log_file is None or empty. The current code does `if _dirname: os.makedirs(...)` only after TICK-101? Actually TICK-501 added null-guard for conn but not for logger. Audit says still open at 2026-08-22. Add robust guard: if not _dirname or _dirname == "" then skip makedirs, and handle log_file being a bare name by using current directory or config_dir/logs. Also ensure file handler creation handles OSError gracefully and falls back to StreamHandler.
  source_documents:
    - "docs/reviews/AUDIT_REPORT.md"
  acceptance_criteria:
    - "GIVEN setup_logger(log_file='app.log') with bare filename WHEN called THEN no crash, file created in cwd or fallback, makedirs not called with ''"
    - "GIVEN setup_logger(log_file='') with empty string WHEN called THEN no crash, falls back to default log_file"
    - "GIVEN setup_logger(log_file='/tmp/a/b/c/app.log') with deep path WHEN called THEN makedirs('/tmp/a/b/c') succeeds"
    - "GIVEN existing logger tests (test_logger_stdout_regression.py 9 tests) WHEN this change applied THEN still pass"
verification:
  test_target: "tests/test_logger_bare_path.py [NEW FILE]"
  validation_command: "python -m pytest tests/test_logger_bare_path.py -q"
```
...
if _dirname:
    try:
        ...
        # Fallback to StreamHandler only
```
