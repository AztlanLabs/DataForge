...
# Ticket TICK-708 — Packaging msi/dmg + version sync + native helper (I2+N4)

> **Wave 7** | **Domain:** Build / Packaging | **Depends on:** None
> **Source:** `docs/proposals/INSTALL_UPGRADE_LIFECYCLE.md` §I2+N4, `docs/reviews/ROADMAP.md` WS-G
...
TICKET_ID: TICK-708
WAVE: 7
TITLE: Packaging msi/dmg + version sync + native helper (I2+N4)
```

**Exclusive write files (SOLE writer for Wave 7):**
- `packaging/wix/Product.wxs [NEW FILE]`
- `packaging/dmg/create-dmg.sh [NEW FILE]`
- `pyproject.toml`
...
**Test target:** `tests/test_packaging_msi_dmg.py [NEW FILE]`
**Validation:** `python -m pytest tests/test_packaging_msi_dmg.py -q`
...
- `docs/DEVELOPMENT_GUIDE.md` §Packaging
- `docs/proposals/INSTALL_UPGRADE_LIFECYCLE.md` §I2+N4
...
architectural_context:
  existing_symbols_to_use:
    - "build_exe.py: onedir profile"
    - "packaging/nfpm.yaml: deb/rpm config"
    - "scripts/bump_version.py: version bump"
  breaking_changes: "None — version bump is additive, packaging is new"
requirements:
  summary: |
    Fix I2+N4 + WS-G + version drift: The current packaging has onedir + deb/rpm via nfpm (TICK-303) but no msi (Windows) or dmg (macOS), and version is still 0.1.0 in pyproject.toml:7 and dataforge/__init__.py:15 despite v0.2.0 target in CONSOLIDATED_SPEC and scripts/bump_version.py existing but not run.

    Create packaging/wix/Product.wxs (WiX v3) for msi that installs to Program Files/DataForge and creates Start Menu shortcut, and packaging/dmg/create-dmg.sh (create-dmg) for dmg with Applications symlink and background. Bump pyproject.toml version to 0.2.0 and dataforge/__init__.py __version__ to 0.2.0 via bump_version.py logic, and ensure build_exe.py onedir profile works for msi/dmg packaging. Add native helper stub native/Cargo.toml for future Rust helper (blake3, io_uring) as placeholder.

    Ensure TICK-402 version sync still holds (single source pyproject.toml).
  source_documents:
    - "docs/proposals/INSTALL_UPGRADE_LIFECYCLE.md"
    - "docs/reviews/ROADMAP.md"
  acceptance_criteria:
    - "GIVEN pyproject.toml version WHEN read THEN is 0.2.0 and dataforge/__init__.py __version__ is 0.2.0"
    - "GIVEN packaging/wix/Product.wxs exists WHEN wix --version called (if installed) THEN validates without error (or graceful skip if wix not installed)"
    - "GIVEN packaging/dmg/create-dmg.sh exists WHEN ls -l THEN is executable and contains create-dmg command"
    - "GIVEN build_exe.py onedir WHEN run THEN dist/onedir/DataForge still builds correctly"
    - "GIVEN existing packaging tests WHEN this change applied THEN still pass"
verification:
  test_target: "tests/test_packaging_msi_dmg.py [NEW FILE]"
  validation_command: "python -m pytest tests/test_packaging_msi_dmg.py -q"
```
...
```python
# Product.wxs: WiX XML for msi
# create-dmg.sh: bash script for dmg
