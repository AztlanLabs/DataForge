...
# Ticket TICK-101 — Route console logger to stderr so CLI JSON stays clean

> **Wave 1** | **Domain:** Core / Logger | **Depends on:** "TICK-004"
...
**Test target:** `tests/test_logger_stdout_regression.py [NEW FILE]`
**Validation:** `python -m pytest tests/test_logger_stdout_regression.py -q`
...
- `docs/CLI_REFERENCE.md` §`fm --format json` (logger stdout check)
- `docs/CONTRIBUTING.md` §8 (When You Change Code → Update table)
- `docs/CONSOLIDATED_SPEC.md` §2–7 (canonical spec)

> **How to verify you read them:** `grep -rn "logger" docs/` should show every cross-reference you must keep in sync; after your change, re-run the grep and fix stale links. Never cite a review as open without `Read`ing the file at `path:line`.

---

# Prompt: Parallel Ticket Agent — DataForge Hardened Backlog

> **Generic prompt for AI agents working on `docs/PARALLEL_BACKLOG.md` (and `docs/proposals/PERFORMANCE_TICKETS.md` aliases) in parallel. Copy this prompt, replace `TICK-101` with the ticket you own, and run. One ticket = one branch = one commit = one PR. Do not touch files outside your ticket's `exclusive_write_files`.**
...
```
TICKET_ID: TICK-101   # e.g. TICK-102 or PERF-101 (alias — see alias map)
WAVE:      1        # from Concurrency Map in docs/PARALLEL_BACKLOG.md
```

If `TICK-101` starts with `PERF-`, resolve its alias first (`PERF-100→TICK-004`, `PERF-101→TICK-102`, `PERF-103→TICK-104`, etc. — see `PARALLEL_BACKLOG.md` Appendix). Never run a `PERF-*` and its `TICK-*` twin in the same wave.
...
git checkout -b feat/TICK-101-kebab-summary
...
git push -u origin feat/TICK-101-kebab-summary
...
**Start now:** Replace `TICK-101` with your assigned ticket (e.g. `TICK-102`), locate its Work Package in `docs/PARALLEL_BACKLOG.md`, and execute steps 1–5 above. For perf aliases, resolve the alias first and work under the `TICK-*` exclusive list.


---

## Work Package YAML for TICK-101 (from `docs/PARALLEL_BACKLOG.md`)
