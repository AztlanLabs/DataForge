...
> **Source:** `docs/reviews/FORENSIC_REVIEW.md` F20
...
TICKET_ID: TICK-704
WAVE: 7
TITLE: F20: locked/in-use files skipped (VSS / acquire)
```

**Exclusive write files (SOLE writer for Wave 7):**
- `dataforge/core/acquire.py [NEW FILE]`
...
**Test target:** `tests/test_acquire_vss.py [NEW FILE]`
**Validation:** `python -m pytest tests/test_acquire_vss.py -q`
...
- `docs/TECHNICAL_SOURCE_OF_TRUTH.md` `core/acquire.py` (new) section
- `docs/proposals/NATIVE_OS_API_REVIEW.md` §VSS
...
architectural_context:
  existing_symbols_to_use:
    ...
    - "recovery.py: scan_trash, carve_files_from_image"
  breaking_changes: "None — acquire is additive, recovery falls back to direct open if VSS not available"
requirements:
  summary: |
    Fix F20: Locked/in-use files are skipped with no VSS/Volume Shadow Copy, no acquire abstraction. On Windows, files locked by another process (e.g., Outlook PST) are missed; on Linux/macOS, permission-denied or busy files are skipped.

    Create dataforge/core/acquire.py with acquire_file(path) -> context manager that on Windows tries VSS via vssadmin / win32api CreateFile with FILE_SHARE_READ, on Linux tries `sudo` cp or `dd` with O_RDONLY, and on all platforms falls back to direct open with retry. Use this in recovery.py's scan_trash and carve paths, and expose as acquire_provider for scanner to optionally use when FileNotFound/PermissionError occurs. Add HAS_VSS flag and graceful fallback.

    Ensure scanner's _scan_single_dir, when encountering PermissionError on a file, tries acquire_file fallback before skipping, and logs with logger.warning.
  source_documents:
    - "docs/reviews/FORENSIC_REVIEW.md"
  acceptance_criteria:
    - "GIVEN locked file on Windows (simulate via mock win32api) WHEN acquire_file called THEN returns readable handle via VSS path"
    - "GIVEN permission-denied file on Linux WHEN scanned with acquire fallback THEN file is still hashed/scanned, not skipped"
    - "GIVEN no VSS available WHEN acquire_file called THEN falls back to direct open and returns handle"
    - "GIVEN existing scanner tests WHEN this change applied THEN still pass (no regression)"
verification:
  test_target: "tests/test_acquire_vss.py [NEW FILE]"
  validation_command: "python -m pytest tests/test_acquire_vss.py -q"
```
...
```python
# acquire.py
def acquire_file(path: str):
    # Try VSS on Windows, else direct open with retry
    # Return context manager yielding file object
    pass

# recovery.py: wrap open(path) with acquire_file
```
