...
> **Source:** `docs/reviews/FORENSIC_REVIEW.md` F10, F16, F21 (dedup)
...
TICKET_ID: TICK-703
WAVE: 7
TITLE: F10/F16/F21: Unicode NFC/NFD + bidi, sparse, reflink dedup
```
...
**Test target:** `tests/test_unicode_sparse_reflink.py [NEW FILE]`
**Validation:** `python -m pytest tests/test_unicode_sparse_reflink.py -q`
...
- `docs/TECHNICAL_SOURCE_OF_TRUTH.md` `core/scanner.py`, `common.py`, `hasher.py` sections
...
architectural_context:
  existing_symbols_to_use:
    - "common.py: FileEntry, st_blocks"
    - "scanner.py: scan_directory, _scan_single_dir"
    - "hasher.py: get_file_hash, _get_block_size"
    - "duplicates.py: find_duplicates, hardlink_key"
  breaking_changes: "None — normalization is additive, st_blocks check is non-breaking"
requirements:
  summary: |
    Fix F10: NFC/NFD + bidi not handled — scanner and duplicates treat file names as raw str, so macOS NFD (e + ◌́) vs NFC (é) are considered different files, and RTL override (U+202E) can spoof extensions. Add unicodedata.normalize("NFC", path) in scanner's _scan_single_dir and in common.py FileEntry normalization, and detect bidi controls (RLO/LRO/RLE/PDF etc.) via regex and flag as bidi_suspicious in FileEntry.

    Fix F16: Sparse files not detected — common.py already has st_blocks, scanner populates it, but hasher and duplicates never check st_blocks*512 < st_size to detect sparse. Add sparse detection in hasher (skip hashing holes, or hash only allocated blocks via FIEMAP/SEEK_HOLE fallback) and in duplicates (treat sparse files as distinct or verify with st_blocks).

    Fix F21 dedup remainder: hardlink dedup DONE in TICK-502, but reflink/btrfs/xfs FICLONE not checked. Add reflink detection via st_blocks vs st_size and via ioctl FICLONE check (or xfs_bmap) and mark reflink_suspicious in FileEntry; duplicates group should consider reflink clones as potential dedup candidates but not as hardlinks.
  source_documents:
    - "docs/reviews/FORENSIC_REVIEW.md"
  acceptance_criteria:
    - "GIVEN file name with NFD 'e\\u0301' (e + combining) WHEN scanned THEN FileEntry.normalized_path is NFC 'é' and bidi_suspicious is False, but file with U+202E is flagged bidi_suspicious=True"
    - "GIVEN sparse file created via truncate -s 1G + dd 1M hole WHEN scanned THEN FileEntry.sparse is True and hasher skips hole or handles correctly"
    - "GIVEN reflink clone via cp --reflink=always WHEN scanned THEN FileEntry.reflink_suspicious is True or hardlink_key handling distinguishes"
    - "GIVEN existing 16 scanner tests WHEN this change applied THEN still pass (no regression)"
verification:
  test_target: "tests/test_unicode_sparse_reflink.py [NEW FILE]"
  validation_command: "python -m pytest tests/test_unicode_sparse_reflink.py -q"
```
...
```python
# common.py: add fields to FileEntry
import unicodedata
def normalize_path(path: str) -> str:
    nfc = unicodedata.normalize("NFC", path)
    # bidi controls
    bidi_chars = set("\u202A\u202B\u202C\u202D\u202E\u2066\u2067\u2068\u2069\u202A")
    bidi = any(c in bidi_chars for c in path)
    return nfc, bidi

# scanner.py: use normalize in _scan_single_dir
# hasher.py: check sparse via os.stat st_blocks*512 < st_size
# duplicates.py: use FileEntry.sparse/reflink_suspicious in grouping
```
