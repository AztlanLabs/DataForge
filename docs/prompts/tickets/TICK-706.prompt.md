...
# Ticket TICK-706 — Engine FTS index + incremental watch (PERF E)

> **Wave 7** | **Domain:** Engine / Index | **Depends on:** None
> **Source:** `docs/proposals/PERFORMANCE_INVESTIGATION.md` §E, `docs/reviews/FORENSIC_REVIEW.md` F15
...
TICKET_ID: TICK-706
WAVE: 7
TITLE: Engine FTS index + incremental watch (PERF E)
```
...
**Test target:** `tests/test_engine_index.py [NEW FILE]`
**Validation:** `python -m pytest tests/test_engine_index.py -q`
...
- `docs/TECHNICAL_SOURCE_OF_TRUTH.md` `engine/` section
- `docs/proposals/PERFORMANCE_INVESTIGATION.md` §E
...
architectural_context:
  ...
  breaking_changes: "None — additive engine, no existing API change"
requirements:
  summary: |
    Implement PERF E: Engine FTS index + incremental watch. The current engine has no full-text index and no incremental file watch — every search does a live scan_directory. Create dataforge/engine/index.py with SQLite FTS5 index (path, name, content_trigram, mtime) that is populated via scan_directory and updated incrementally via inotify/FSEvents/USN Journal/watchdog (gated: import watchdog if available, else polling fallback). Provide Index class with build(path), search(query), update(path), and watch(path, callback) methods. Ensure thread-safety and that existing search still works without index (fallback).

    Also handle F15 budget: keyword_search already has 10MB per file limit from TICK-109, but global budget across workers is still per-file. Ensure index search respects global byte budget.
  source_documents:
    - "docs/proposals/PERFORMANCE_INVESTIGATION.md"
    - "docs/reviews/FORENSIC_REVIEW.md"
  acceptance_criteria:
    - "GIVEN empty index WHEN build('/tmp/test') called THEN FTS index contains all files from scan_directory"
    - "GIVEN indexed dir WHEN search('foo') called THEN returns matching FileEntry via FTS without live scan"
    - "GIVEN file changed on disk WHEN watch callback fires THEN index updates incrementally without full rebuild"
    - "GIVEN no watchdog installed WHEN watch called THEN falls back to polling every 5s"
...
class Index:
    ...
    def watch(self, path, callback): ...  # watchdog or polling
```
