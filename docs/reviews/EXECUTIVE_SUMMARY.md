...
> **2026-07-12 update:** WS-C (Interaction Correctness) and WS-D (IA, Naming & Parity) are now **shipped** — 7 interaction fixes (file-vs-folder riddle, settings autosave, dark-mode dedup, progressive disclosure, destructive checklist, named busy task, rich help) and 5 IA/label/parity changes (task-oriented sidebar, Automations merge, label renames, `fm devices` GUI, stray-name sweep) all landed. The next open phase is **WS-E** (Motion, Empty/Error, A11y — items 2e.1–2e.7). The test count is **276** (was 255 at the start of WS-C).
...
1. **✅ The test suite runs green — 276 tests pass.** Original collection failure (`rename_with_regex` removed) is fixed. All correctness bugs (H1, M1–M6, L1–L9) are resolved. 30 new token-regression tests guard the design system; the WS-C and WS-D work added another 21 contract regressions.
...
4. **✅ UX is now organised around user tasks, not developer modules.** The task-oriented sidebar (Home / Find & Organize / Clean & Optimize / Recover & Investigate / System) replaces the old module-oriented Overview / File Utilities / System Maintenance / Advanced Analysis / Application split. The file-vs-folder riddle is gone (2c.1), settings autosave with a transient "Saved ✓" (2c.2), the dark-mode control is single-source-of-truth (2c.3), the sidebar shows every group regardless of tier (2c.4), destructive previews are scrollable checkable tables (2c.5), the status bar names the running task (2c.6), and view help renders Markdown (2c.7). Tools & Workflows + Action Builder are merged into **Automations** (2d.2), `fm devices` is now a GUI view (2d.4), and labels are renamed to user-facing names (2d.3 / 2d.5).
...
| Sidebar animation + Braille spinner replacement + focus ring + icons (2e) | ⏳ Open (WS-E) |

---

## Overall assessment

**Solid engineering under a good architecture.** The correctness backlog is cleared and 276 tests are green. CI is wired (`.github/workflows/ci.yml` runs pytest + ruff + mypy + pip-audit on every push/PR), the S1–S13 security backlog is closed, and the user-facing IA/label/parity work (WS-C and WS-D) is shipped. What remains is the residual forensic-soundness architecture (chain-of-custody, Evidence Mode, provenance, UTC timestamps) tracked in **[`FORENSIC_SECURITY_REVIEW.md`](./FORENSIC_SECURITY_REVIEW.md)** as F1–F21, the WS-E motion/empty/error/a11y polish, and the WS-F architecture consolidation.
...
- [x] Run `PYTHONPATH=. pytest -q` on every PR via CI (must pass all tests; 276 currently green)
...
- [x] `pytest` collects and passes all 276 tests. — [x] Coverage tracked.
