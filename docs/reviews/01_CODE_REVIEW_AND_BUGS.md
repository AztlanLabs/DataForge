...
**Scope:** `dataforge/` (core, modules, ui, cli), `tests/`, packaging.
...
- **Evidence:**
  ...
  from 'dataforge.core.operations.files'
  ```
  `test_comprehensive.py` imports `rename_with_regex` (and calls it at lines 723/727/744/748), but that symbol does not exist anywhere in `dataforge/`. Only `FileActionService.rename_items_with_regex` and the low-level `rename_path` exist.
...
- **Where:** `dataforge/core/hasher.py:11-12` vs `dataforge/cli.py:580` and `dataforge/modules/forensics.py:37`
...
- **Where:** `dataforge/modules/integrity.py:110-114`
...
- **Where:** `dataforge/core/scanner.py:54,62,66,72`
...
- **Where:** `dataforge/modules/integrity.py:70,134` (`_hash_worker(entry_path, "md5", ...)`)
...
- **Where:** `dataforge/core/cache.py:18` (`check_same_thread=False`, single global `file_cache`)
...
- **Where:** `dataforge/modules/duplicates.py:136,213` + `core/config.py` (`hash_algorithm: "md5"`)
...
- **Where:** `dataforge/core/actions/filters.py:59,61,100,142,144,162`; `core/actions/media.py:12`
...
- **Where:** `dataforge/core/media_ops.py:68`; `dataforge/ui/plugin_loader.py:52`
...
- **Where:** `dataforge/core/media_ops.py:144-145`
...
- **Where:** `dataforge/modules/metadata.py:60` (`HAS_EXIFTOOL = _exiftool_available()`)
...
- **Where:** `dataforge/core/operations/files.py:162-178`
...
- **Where:** `dataforge/modules/recovery.py:182-188` (`_scan_windows_trash` returns `[]`)
...
- **Where:** `dataforge/modules/password_tools.py:382-388` (`"success": True` unconditionally)
