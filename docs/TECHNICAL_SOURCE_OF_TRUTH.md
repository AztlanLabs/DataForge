...
**Last verified:** 2026-08-23 09:00 UTC *(re-verified against `dataforge/` HEAD `b373e7e`; Wave 5+6 12/12 DONE, 1213 tests, Wave 7 (8 tickets) 🔜 READY + dc44be4 UI fixes (dropdown, tier, STOP, artifacts) — added `core/image_io`/`streams`/`logger` chain, `modules/indicators`/`sanitisation`, `engine/parsers`; see `DOCUMENTATION_AUDIT_2026-08-22.md` §1.4)*
...
This module now imports and passes — `rename_with_regex` was restored in `dataforge/core/operations/files.py`. The whole suite collects and runs green (723 tests). The coverage list below is accurate. See [`docs/reviews/AUDIT_REPORT.md`](./reviews/AUDIT_REPORT.md) for verification details.
...
The structural facts below are still accurate. Most *defects* from the 2026-07-10 review are now **fixed**; still-open *security risks* are tracked in [`docs/reviews/AUDIT_REPORT.md`](./reviews/AUDIT_REPORT.md) with severities and line-level evidence.
